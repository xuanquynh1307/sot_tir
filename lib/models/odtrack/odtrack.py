...
class ODTrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, cls_head, aux_loss=False, head_type="CORNER", token_len=1):
    ...
    def forward(self, template: torch.Tensor,
        ...
        out.update(aux_dict)
        out['backbone_feat'] = x
        # Forward cls head (calculate confidence score)
        out.update({'pred_logits': self.cls_head(x[:, :1])})
        # out_dict.append(out)
...
def build_odtrack(cfg, training=True):
    ...
    # pretrained_path = '/content/drive/MyDrive/SOT/ODTrack/pretrained_networks'
    ...
    cls_head = MLP(768, 768, 1, 3)
    # cls_head = MLP(1024, 1024, 1, 3)
