    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            if not self.use_amp:
                ...
                with autocast():
                    loss, stats = self.actor(data)
            loss_list['giou_loss'].append(stats['Loss/giou'])
            loss_list['l1_loss'].append(stats['Loss/l1'])
            loss_list['focal_loss'].append(stats['Loss/location'])
            loss_list['cls_loss'].append(stats['Loss/cls'])
    ...
    def train_epoch(self):
        ...
        for loader in self.loaders:
            if self.epoch % loader.epoch_interval == 0:
                ...
                loss_dict.update({loader.name: self.cycle_dataset(loader)})
                # import pdb
                # pdb.set_trace()
