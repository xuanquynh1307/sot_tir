        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    # Sample test and train frames in a causal manner, i.e. search_frame_ids > template_frame_ids
                    while search_frame_ids is None:
                        # sample first template frame
                        base_frame_id = self._sample_visible_ids(visible, num_ids=1,
                                                                 min_id=self.num_template_frames - 1,
                                                                 max_id=len(visible) - self.num_search_frames)

                        # sample remain template frame
                        prev_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,
                                                                  min_id=base_frame_id[0] - self.max_gap - gap_increase,
                                                                  max_id=base_frame_id[0])
                        if prev_frame_ids is None:
            ...
            try:
                ...
                if random.random() < self.pos_prob:
                    label = torch.ones(self.num_search_frames, ).reshape(-1, 1)
                    search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids,
                                                                                   seq_info_dict)
                    search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros(
                        (H, W))] * self.num_search_frames
                else:  # negative samples
                    label = torch.zeros(self.num_search_frames, ).reshape(-1, 1)
                    if is_video_dataset:
                        search_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_search_frames, force_invisible=True)
                        if search_frame_ids is None:
                            search_frames, search_anno, meta_obj_test = self.get_one_search()
                        else:
                            if len(search_frame_ids) > 1:
                                search_frame_ids.sort(reverse=False)  # Sort from small to large
                            search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids,
                                                                                           seq_info_dict)
                            search_anno["bbox"] = [self.get_center_box(H, W)] * self.num_search_frames
                    else:
                        search_frames, search_anno, meta_obj_test = self.get_one_search()
    ...
    def get_one_search(self):
        ...
        if is_video_dataset:
            if self.frame_sample_mode == "stark":
                search_frame_ids = self._sample_visible_ids(seq_info_dict["valid"], num_ids=self.num_search_frames)
            else:
                search_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_search_frames, allow_invisible=True)
        else:
